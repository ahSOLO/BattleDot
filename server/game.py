...
def user_action(data, user):
    ...
    if data["action"] == "exit":
        ...
        if STATE["game_state"] == GameState.SETUP_BOARDS:
            ...
            loop.run_until_complete(helpers.send(attacking_player, json.dumps({"type" : "notification", "content": "opponent left", "state": STATE["game_state"] })))
    else:
    ...
    if STATE["game_state"] == GameState.LOBBY:
        ...
        if STATE["entering_state"]:
            ...
            for player in PLAYERS:
                ...
                loop = asyncio.new_event_loop()
                loop.run_until_complete(helpers.send(player, json.dumps({"type" : "user_message", "content": f"Your ship is positioned at {ship_position}!"})))
        ...
        if STATE["entering_state"]:
            ...
            for key, value in game_boards_copy.items():
                ...
                if STATE["attacks"][key] == value:
                    ...
                    loop = asyncio.new_event_loop()
                    loop.run_until_complete(helpers.send(player, json.dumps({"type" : "notification", "content": "you lose" })))
                    STATE["game_boards"].pop(key)
            STATE["entering_state"] = False
        if len(PLAYERS) == 1:
            winner = PLAYERS.pop()
            loop = asyncio.new_event_loop()
            loop.run_until_complete(helpers.send(winner, json.dumps({"type" : "notification", "content": "you win" })))    
